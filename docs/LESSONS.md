...
**[2026-09-03] Problem:** Adding a user on a remote CCR router that
...
**[2026-09-05] Problem:** The GitHub Actions `validate` job failed with `ModuleNotFoundError: No module named 'respx'` during pytest collection because `respx` was present in the local virtualenv for RouterOS client mocking but missing from the CI pip install step (`pip install pytest pytest-asyncio ruff`).
**→ Solution:** Added `respx` to the CI install step in `.github/workflows/release-docker.yml` (`pip install pytest pytest-asyncio respx ruff -r backend/requirements.txt`).


**[2026-09-06] Problem:** `git status` failed with `fatal: .git/index: index file smaller than expected` and every git command in the repo was unusable. `.git/index` was 0 bytes; the index format needs at least a 12-byte header plus a 20-byte trailing SHA, so an empty file is rejected before any entry is read. No `index.lock` remained. The working tree also carried four bogus modifications that nobody made: a duplicated `pip install` line in `.github/workflows/release-docker.yml` (the pre-`respx` version reinserted above the fixed one), a deleted blank line between `import os` and `import yaml` in `tests/test_github_workflow_syntax.py`, and stray trailing newlines in two more files. Timestamps put the workflow edit one minute before the index truncation, so a single process was writing into both the working tree and `.git` and died mid-write.
**→ Solution:** The index is derived state and was rebuilt with `git read-tree HEAD` - no history, refs or working-tree file is touched by it, and `git fsck` confirmed objects, refs, reflog and the `v0.2.0` tag were all intact. The bogus edits were put in `git stash` rather than discarded with `git checkout --`, so they stay recoverable if the diagnosis was ever wrong. Lesson: a 0-byte `.git/index` looks catastrophic and is not - it is the one file in `.git` that is fully reconstructible from HEAD. But check the working tree in the same breath: the crash that truncated the index had also silently reverted a committed fix, and since `ruff` runs with rule `I` (isort) here, the un-sorted import block would have failed the release workflow's own `validate` gate.

**[2026-09-06] Problem:** The `linux/arm/v7` leg of the release build could not have succeeded. The earlier multi-arch work (see 2026-09-05) fixed `uvloop`, but only `uvloop` - and it is not the only dependency without ARMv7 wheels. Resolving the full `backend/requirements.txt` with `pip download --only-binary=:all: --platform manylinux_2_31_armv7l` shows five more source-only packages, four of which are *hard* dependencies that no marker can skip: `cffi` (pulled in by `cryptography`), `greenlet` (SQLAlchemy asyncio), `MarkupSafe` (Mako/alembic) and `PyYAML`. `python:3.12-slim` has no `gcc`/`g++`/`make` and no `/usr/include/ffi.h` (verified by running the image), so the arm/v7 build aborted at the first source package - `cffi`. This matters because RB4011, the board this deployment targets, is a 32-bit ARMv7 Cortex-A15.
**→ Solution:** Added a throwaway `wheelbuilder` stage carrying `build-essential` + `libffi-dev` that turns requirements into a local wheelhouse; the `runtime` stage installs from it with `--no-index --find-links`, delivered as a `--mount=type=bind` so no wheel layer lands in the shipped image. `httptools` got the same ARMv7-excluding marker `uvloop` already had, since it is likewise an optional accelerator (uvicorn falls back to `h11`). Regression cover is structural and offline: `tests/test_dockerfile_multiarch.py` asserts the builder stage owns a toolchain and the runtime installs offline, and `tests/test_requirements_multiarch.py` now parametrises the marker check over both accelerators. Lesson: "add a platform marker" only fixes dependencies you are *willing to drop*. A hard dependency with no wheel for the target architecture needs a compiler somewhere in the build, and the honest guard is `--no-index` in the runtime stage - it turns a missing wheel into a loud build failure instead of a silent attempt to compile inside an image that has no compiler.

**[2026-09-06] Problem:** Switching routers left the telemetry sparklines drawing the previous router's data. `TelemetryBar` keeps five rolling buffers (rx, tx, CPU, memory, temperature) of up to 60 samples in component state and nothing emptied them when the selection changed, so the new router's ticks were appended to the old router's curve and it took a full minute to scroll out. The damage was worse than staleness: the traffic sparklines have no fixed ceiling and scale to `Math.max(...values, 1)`, so a router that had peaked at 900 Mbps rescaled the axis of a router idling at 60 Kbps and flattened it into a straight line at the bottom of the tile. `useWebSocketTelemetry` already blanks its frame on a router change, but that could not do the clearing on its own - the push effect starts with `if (!router) return;`, so the very tick that signalled the switch was skipped instead of used as the reset.
**→ Solution:** A separate effect keyed on `activeRouter?.id` empties all five buffers, declared before the push effect so that a reset and the new router's first sample landing in one commit happen in that order. Covered by `TelemetryBar.test.jsx`, which drives two frames from a fast router, switches, and asserts no sparkline SVG survives - one sample is not a curve, so any SVG still rendered is the old router's. Lesson: an early `return` on a null/empty input silently discards the one event that marks a boundary. When state is scoped to an entity, reset it on that entity's id changing rather than hoping a falsy frame will be handled somewhere downstream - and check whether the widget auto-scales, because mixing two entities' data can corrupt the axis long before anyone notices the values themselves are foreign.

**[2026-09-06] Problem:** The firmware modal's release notes were empty on every non-stable channel, and said "No lines match the filter." over an empty filter box. Two defects stacked. The first: `VERSION_REGEX` in `backend/app/services/changelog.py` was `^\d+\.\d+(\.\d+)?$`, so a development-channel target like `7.25beta3` failed `validate_version` and the endpoint answered 400 before any request left the process - even though MikroTik does publish those notes at the ordinary URL (`upgrade.mikrotik.com/routeros/7.25beta3/CHANGELOG` answers 200 with 16 KB). The second: the frontend's `.catch(() => setChangelog(null))` discarded the reason, and the renderer treated "no lines" as one state, so a failed fetch and a filter that excluded everything produced the same sentence - one that blamed the reader for a filter they had not typed in.
**→ Solution:** The pattern now accepts an optional pre-release suffix, `^\d+\.\d+(\.\d+)?((alpha|beta|rc)\d+)?$` - still fully anchored and still admitting nothing but digits, dots and one of three known words, because the value is interpolated into an upstream URL. `tests/test_changelog_service.py` covers the pre-release forms alongside the existing traversal and injection cases. The modal keeps a `changelogError` and renders three distinct states - failed (with the server's own message), loaded-but-filtered-out, and loaded. Lesson: a validator built from the examples in front of you silently becomes a feature gate for the cases you did not have. And when a catch block drops the error, the empty state it produces gets attributed to whatever else could make the view empty - here, a filter nobody touched, which sent the search for the bug to the wrong end of the stack.

**[2026-09-06] Problem:** The header tagline rendered as "RouterOS Compan…" in English and "Компаньон Route…" in Russian. It had been pinned to the app name's own rendered width to stop it widening the header; the pin worked, but "MikroMan" at 20px bold measures about 90px while "RouterOS Companion" at 10px needs about 94px, so the tagline lost its last word to a four-pixel shortfall. The Russian translation, being longer still, clipped harder.
**→ Solution:** The tagline keeps `position: absolute` - that, not the width pin, is what actually guarantees the header cannot grow, since an out-of-flow element contributes no width to its parent. Only the right-edge pin was dropped, in favour of `width: max-content` with a `max-width` guard, so the text renders in full and overhangs the app name by those few pixels at no layout cost. `app_subtitle` is also no longer translated: it is the product's own name, and the Russian rendering was a longer string for no gain. `translations.test.js` pins it to "RouterOS Companion" in every language. Lesson: when a fix must not affect layout, take the element out of flow and stop there - additionally constraining its width solves a problem that being out of flow had already solved, and the constraint is what breaks the content.

**[2026-09-06] Problem:** Every router backup failed - 51 of 51 - alternating between "Timed out waiting for <file> to settle on router flash" and "'utf-8' codec can't decode byte 0x80 in position 15". Two defects in the same file. `_wait_for_file_settled` polled with a bare `GET /file`, which RouterOS answers with each file's `contents` included, so the listing carried the raw bytes of every binary on flash - including the `.backup` the poll was waiting for - and the body could not be decoded; the poll's `except Exception: pass` swallowed that and the loop ran to its deadline. The sibling method `sweep_temporary_files` already carried a docstring explaining this exact trap and a `.proplist` to avoid it; the settle poll had simply never been given one. Separately, `create_system_backup` read chunks with `response.json()`, which hands the raw body to `json.loads` and so insists on UTF-8 - the first non-ASCII byte of a binary backup aborted the read.
**→ Solution:** The settle poll now requests `.proplist=.id,name,size`. Chunk bodies go through a `read_chunk_body` helper that falls back to `json.loads(resp.content.decode("latin-1"))` on a `UnicodeDecodeError` - latin-1 maps bytes one-to-one and is already the encoding each chunk is re-encoded with. Both are covered in `tests/test_routeros_backup_transport.py`; the existing tests missed the first because they mocked `_wait_for_file_settled` out entirely. Lesson: when a fix is written up as a docstring on one method, grep the file for every other call that shares the shape - the note explained the trap perfectly and the neighbouring method still walked into it. And an `except Exception: pass` inside a retry loop converts a precise error into a timeout, which sends the next reader looking at latency instead of at decoding.

**[2026-09-06] Problem:** "Hide own logins" never hid anything. `is_self_api_login` required the log line's source address to equal the address MikroMan discovered for itself with a UDP-connect probe. MikroMan runs in a container: the probe returns the container's address (172.17.x.x) while the router, on the far side of NAT, records the host's - so the comparison could not succeed. The regex also demanded a `from <ip>` segment, which meant the address-less sibling line RouterOS writes for the same event was unmatchable in principle.
**→ Solution:** Matching is on the account name alone, over both line shapes. The guarantee that was lost - an api session opened by someone else holding the same credential is now hidden too - is stated in a footnote on the setting itself rather than left implicit; logins over winbox, ssh and webfig are untouched, so a human using the account stays visible. Lesson: a filter that compares an address the process cannot observe is not conservative, it is inert - and an inert filter is worse than a blunt one, because the UI still claims it is doing something. When correctness depends on a value the deployment cannot supply, say so at the setting, not in a comment.

**[2026-09-06] Problem:** The logs badge sat on "99+" permanently and reading the log could not clear it. `/logs/stats` counted a fixed trailing 24 hours, so the badge was a daily error total - on a router logging a few hundred routine errors a day it conveyed nothing and trained the reader to ignore it. Related: the navbar firmware badge kept advertising a version from a channel the router had already left, because `firmwareStatus` is refreshed on App's five-minute slow poll and the modal - which had just fetched the correct answer while switching the channel - kept it to itself.
**→ Solution:** `/logs/stats` takes an optional `since`; the app stores when this viewer last read each router's log in `localStorage` and passes it, stamping the marker when the viewer is closed. The firmware modal gained an `onStatusChange` callback, routed through a single `applyStatus` so every fresh status from the server - initial fetch, update check, channel switch, post-reboot reconnect - reaches the badge at once. Lesson: a badge is a claim about what the reader has not yet seen. Counting a fixed window instead makes it a metric, and a metric that cannot be acted on gets ignored - which costs the badge its purpose for the day it is actually needed. Where a component holds a fresher copy of state that something else displays, hand it over rather than waiting for the poll to rediscover it.

**[2026-09-07] Problem:** Firmware upgrade with bootloader staging returned 500 even though the router successfully updated. The sequence was backwards: Gate 4 tried `upgrade_routerboard_firmware()` before Gate 5's `install_package_update()`, but RouterBOOT firmware is bundled with the package update - it is not available until after the package is installed and the router reboots. Additionally, both `install_package_update()` and `upgrade_routerboard_firmware()` call `resp.raise_for_status()` after POSTing to endpoints that trigger immediate reboots, so the router drops the connection before sending a response and the call raises `httpx.ReadTimeout` - which is actually the expected behaviour (command received, reboot in progress), not a failure.
**→ Solution:** Reversed the sequence: package install first (triggers reboot), wait for router to come back online (poll `/system/resource` with 5-minute timeout), then check if bootloader firmware is available and upgrade if so (triggers second reboot). Both transport methods now catch `ReadTimeout`, `ConnectError` and `RemoteProtocolError` and treat them as success - the command was received and the reboot is in progress. The endpoint returns `bootloader_upgraded: bool` so the UI can report what actually happened. Lesson: when an API endpoint triggers an immediate reboot, the connection drop is the success signal, not an error to propagate. And when two operations have a dependency (bootloader firmware arrives with the package), the order matters more than the gate numbers suggest.

**[2026-09-07] Problem:** `test_quota_used_bytes_is_scoped_to_the_active_router_not_summed_across_all` failed with `used_bytes=0` instead of `10_000_000_000`. The test added a `RouterTrafficRollup` for `date.today()` (local timezone), but `build_quota_status` queries traffic up to `router_local_now(...).date()` which with offset=0 returns UTC date. When the test runs in a timezone ahead of UTC (e.g., Europe/Moscow at UTC+3), `date.today()` is tomorrow in UTC, so the rollup date falls outside the query range and the result is 0.
**→ Solution:** Use `datetime.now(timezone.utc).date()` instead of `date.today()` when the test sets router offset to 0 and expects the router-local clock to match UTC. Lesson: when testing time-dependent logic that uses a configurable clock offset, the test data must use the same clock source as the code under test. A test that mixes local time with UTC-offset time will pass or fail depending on the timezone of the machine running it.

**[2026-09-07] Problem:** Once the graphs started showing peaks, the 7-day and 24-hour charts looked broken: broad smooth humps and what read as "holes in the data". The working theory was that the container was losing the router. It was not. Three routers on three independent networks (LAN `192.168.123.1`, plus two public REST endpoints) stopped writing samples within ~13 s of each other and restarted within ~28 s, seven times in seven days, every stop in the evening and every restart between 07:52 and 10:04; the OS itself had nothing to say about it. Two red herrings cost time: `router_logs` has ~150 rows whose *event* timestamps fall inside a 17-hour hole, which looks like the collector ran all night - but all of them carry `created_at` = one second (1414 rows in 8 s at 08:28:57), i.e. the `/log` buffer drained on resume; and the hourly view of that table showed no rows before that burst because `router_logs` is pruned, so the absence of evidence briefly looked like evidence of absence.
**→ Solution:** Prove the observer before investigating the observed. The host runs in WSL2, and three signals line up: `journalctl` (whole-OS: kernel, cron, systemd) is silent for 5.7 h ending 07:52:12 while telemetry resumed at 07:52:47 - 35 s later; the log scraper is a *separate* asyncio task on its own 60 s interval and it paused at the same instants (`17:12:19 → 10:03:47`, `23:49:17 → 07:52:35`); and `RestartCount=0` with a `/proc/uptime` shorter than wall time since the last dockerd start means nothing crashed, the VM was frozen. So the boxes sleep, the collector sleeps with them, and 94 of 168 hourly buckets is all the 7-day chart can honestly have. Two real defects were fixed in the same pass. Rendering: `timeSegments()` in `frontend/src/utils/chartScales.js` ends the current run at any gap wider than 2.5 buckets, because both responses now carry `bucket_seconds` (`60` default) so an empty bucket is distinguishable from an unmeasured hour - a filled peak band interpolated straight across a hole draws a mountain over hours nobody sampled, and a lone surviving bucket is drawn as a dot rather than vanishing. The range header prints `last sample <router-local time>`, since a stale "current" figure is otherwise indistinguishable from a live one. Logging: a frozen router produced *no* line at all, because `collect_and_store` caught everything at `logger.debug` and the per-router block in `background_sync_worker` did the same; both now report one WARNING per distinct fault and one INFO on recovery (`_note_collect_failure` / `sync_failures`), since warning-per-tick would bury the log within minutes. Lesson: when several independent sources fail and recover in the same second, the common factor is whatever watches them - check the machine before the links. Event timestamps are not liveness proof; compare them against ingestion time, because a post-freeze buffer drain is indistinguishable from continuous polling otherwise. And a filled area asserts something about every x it covers, so any time-proportional axis must come with gap detection - placing points correctly is what makes the old bridging visible.
**→ Also recorded:** these freezes do not lose traffic *volume*. RouterOS byte counters are cumulative, so the first sample after a freeze carries everything that moved meanwhile (ether1 across the 8-hour freeze: 127,472,850,167 → 133,163,148,718 = +5.69 GB credited), `rollups.split_bytes_by_day()` pro-rates that delta across the calendar days it spans, and per-device mangle counters are read with the same cumulative logic - the loss is rate resolution (no `rx_rate_bps` samples exist for those hours) and within-gap timing, not bytes. Bytes *are* lost when the counter decreases inside a blind window (`max(0, cur - prev)` discards it), so a reboot during an outage forfeits the interval: check `lag(rx_bytes_total)` for decreases and whether they are multi-interface-in-one-second (reboot) or one dynamic tunnel (link flap).

**[2026-09-07] Problem:** The Router Health graphs "flattened the peaks" - a link that carried 343.6 Mbps showed a 30-day axis topping out at 21.8 Mbps. `MetricsCollector.get_interface_history` averaged each display bucket (`sum(...) / len(...)`) with a 14 400-second bucket on the 30-day range against a 10-second sample cadence, so 1 440 samples collapsed into one mean and a one-minute burst arrived as 0.83 Mbps. Nothing downstream could recover it: `MetricCharts` derived the axis from `Math.max(...points.map(p => p.rx_rate_bps))`, i.e. from numbers that no longer contained the event. Measured on a copy of the live database: raw `max(rx_rate_bps)` = 343.6 Mbps, bucket-mean maximum = 9.5 Mbps. The same averaging also made `current_rx_bps` report a four-hour average under a "live" label, and the 30-day read pulled every raw row through the ORM to average it in Python.
**→ Solution:** Each bucket now reports its mean *and* its worst case. `backend/app/services/metrics_collector.py` aggregates in SQL instead - `_time_bucket()` builds a `CAST(strftime('%s', timestamp) AS INT) / bucket_seconds` grid (integer division floors, and `strftime` reads the naive-UTC column as UTC, which is what the Python helper it replaced guaranteed explicitly), and interface rates are grouped per sample instant first so the summed series is what gets peaked: a bucket where ether1 spiked at 200 Mbps and ether2 spiked at 50 Mbps one sample later peaks at 200 Mbps, not 250 Mbps. `InterfaceRatePoint` gained `rx_peak_bps`/`tx_peak_bps` and `SystemMetricPoint` gained `cpu_peak`/`temperature_peak`/`voltage_min`/`voltage_max`, all defaulted so `current_*` and the old mean fields keep their meaning. `MetricCharts` scales on `chartMax(points, ['rx_peak_bps', 'rx_rate_bps'], …)` and draws a translucent peak band under the average line (min–max band on the voltage view), `current_*` reads the newest single sample, and `xPositions` places points by timestamp so an outage stays a hole instead of sliding later spikes left. Two extra reasons to keep this shape: the history charts had been building curves with a midpoint cubic (Catmull-Rom), which overshoots and would have started *inventing* traffic once real spikes arrived - they now share `utils/sparkline.js`'s monotone Hermite, whose docstring already says exactly that about the live tiles; and a downsampling test with one sample per bucket cannot fail, so the regression tests in `tests/test_metrics_and_graphs.py` put ≥2 divergent samples inside a bucket aligned to a bucket edge. Lesson: averaging is not downsampling - a mean answers "what did it settle at" and never "what did it carry", so any chart whose purpose includes bursts must keep an extreme per bucket. And when the axis is computed from the plotted series, discarding peaks corrupts the scale of everything that survives alongside them.

**[2026-09-07] Problem:** Running MikroMan *on* the router it manages (RouterOS container package on the hAP be³ Media) had been deferred, and picking it up showed the deferred part was larger than the feature: the app could read `/container`, `/container/mounts`, `/container/envs`, `/container/config` and could add/start/stop a container, but had no way to create a mount, an env list, set `layer-dir`, or provision the bridge/veth/NAT a container needs - so `scripts/setup_ros_container.rsc` was the only path, and measuring it against the live box found three defects: it never moved `layer-dir`/`tmpdir` off internal flash (the image unpacks to a few hundred MB against barely more than that free), its `dstnat` for port 1928 had no `in-interface`, publishing an administrative UI on every interface including WAN, and it mounted `mikroman_data` without that directory existing. Two more facts came from the router itself: `POST /system/script` answers **400 "no such command"**, so REST offers no shell channel (no `mkdir`, no `/import`), and the credentials the script wanted in `/container/envs` are not needed there at all - router logins and the bot token live in the encrypted database (`AppSetting.telegram_bot_token`, `Router.password`), so copying `app.db` carries them, while writing them as env would put them in plaintext in the running config and in every exported `.rsc`.
**→ Solution:** `backend/app/services/container_setup.py` plus `routeros/provisioning.py`: thin write primitives on the client (`/container/config/set`, `/container/mounts/add`, `/interface/veth/add`, `/ip/firewall/nat/add`, `POST /upload`), and a service that snapshots the router's state once (`asyncio.gather`), produces a plan of `create|set|exists|skip|blocked` steps, and applies *that same list* - so what the UI showed is what ran, not a retelling. `POST /routers/{id}/containers/setup/plan` and `/setup/apply` sit **above** the existing `/{container_id}/{action}` route on purpose: FastAPI matches in declaration order, and below it `/setup/plan` parses as container `setup` + action `plan` and gets rejected - a route visible in OpenAPI and unreachable in practice, pinned by `test_the_setup_route_is_not_read_as_a_container_action`. Storage is a required argument with no default, the web forward is only ever created with an `in-interface`, a subnet overlapping an existing `/ip/address` blocks before anything is written, objects without a `mikroman:` comment are refused rather than edited, and a refused command stops the walk while reporting which steps already landed. `migrate-data` snapshots the live database with SQLite's backup API (a byte copy of a file written to every few seconds arrives as "database disk image is malformed") and sends `.secret_key` with it, refusing while the container runs. Environment stays empty by default. Lesson: provisioning is where an idempotent plan beats a script - a script is right for the machine it was written on and wrong silently everywhere else, while a plan that inspects first can say "this bridge is not mine, I will not touch it". Two RouterOS REST rules worth carving in: menu paths need their explicit verb (`POST /x/add`; a bare `POST /x` is "no such command"), and there is no command-execution channel, so anything a CLI would `mkdir` must be produced by an upload instead. And measure the target before writing its instructions: the image size, the free-space figure and the "those secrets are already in the database" fact each inverted a decision that had been made from memory.

**[2026-09-08] Problem:** Finishing the first real deployment - MikroMan running as a container on the hAP be³ Media it manages - produced three more corrections, the third of which was an app bug the deployment merely exposed. `/container/add` refuses `mounts=` outright (`unknown parameter mounts`); the binding attribute on the container row is **`mountlists`**, and the env equivalent is `envlists` - the Containers page had been sending and reading the other names all along, which is why a correctly mounted container showed no mount. Second: RouterOS names a container row after its image (`mikroman:latest`), not after anything the operator picks, so an idempotency check keyed on the requested name would never see what it had created and a second Apply would add a twin. Third and worst: the bot came back **disabled** on the container while the database plainly held `telegram_bot_token` - `TelegramBotService` is constructed from environment settings, and the stored row was consulted only by the `/telegram/test` endpoint. So any restart of any deployment brought the app up with proactive alerts silently off, and only the container made it obvious because a container always restarts.
**→ Solution:** `mountlists`/`envlists` on both sides (`ContainerManager.create` maps the UI's `mounts`/`envlist` onto them; `_to_container` reads them back), the existing-container match keys on the `mikroman:` comment because that is the only field this side owns, `add_container` raises `RouterOSCommandError` so the setup walk reports which step was refused instead of surfacing an anonymous HTTP 500, and `TelegramBotService.load_persisted_settings()` applies the stored rows (and builds the bot) from `start()`, skipping absent or empty rows so an environment value still wins, with an unreadable database leaving the bot off rather than taking the app down. Transfer note for the same deployment: RouterOS has no REST upload and its `/file` listing hides dot-files, so the database and its key went over SFTP with the router's own `rest` account - and `sshpass` needs password-only auth (`PubkeyAuthentication=no -o PreferredAuthentications=password`) plus `scp -O`, because RouterOS's non-standard banner leaves `sshpass` no prompt to match in batch or SFTP mode. Lesson: a deployment on the target device is the only honest test of "the values are stored, so the app must read them back" - a setting saved by one code path and consumed by a different one looks perfect until the process restarts. And when a command's parameter is rejected, read the row back and use the names it reports; guessing from documentation of an older release is how `mounts` survived this long.

**[2026-09-08] Problem:** The first real Apply against the hAP be³ Media stopped with `config: Bad Request`, and after that was fixed it stopped again at `mount: unknown parameter name`. Two different kinds of fault were hiding behind the same two words. The router had answered precisely both times - `detail: "unknown parameter dns-servers"`, `detail: "missing =list="` - but `_detail()` looked at `message` before `detail`, and RouterOS puts "Bad Request" there, so every refusal reached the UI as the same useless string. Underneath it were three API assumptions that had never been checked on a device: `/container/config/set` takes no `.id` (that one is genuine, and passing it fails), `dns-servers` is not an attribute of `/container/config` on 7.24.2, and mounts and envs are grouped by **`list`**, not `name` - the DTOs read `name`, so rows the app had just created successfully displayed as nameless. A fourth assumption died outright: **there is no upload endpoint over REST at all** (`POST /upload` answers "no such command or directory" for JSON and 415 Unsupported Media Type for multipart, octet-stream and form bodies alike), so `upload_file` was code for a channel that does not exist, and the ~100 MB database cannot be pushed the way the configuration was.
**→ Solution:** `_detail()` prefers `detail`. `/container/config/set` is sent without `.id`, and only with the attributes the device itself reported back in its config row - the set is atomic, so one unknown attribute costs every attribute in the request; an unsupported one is named in the step detail instead of being quietly dropped. `add_container_mount`/`add_container_env` send `list=`, and the overview maps `list` into the DTO's `name` (accepting the old key as a fallback). Directory creation goes through `/file/add` with a plain `keep.txt` (a dotted name is refused: "invalid file name"), which is also what makes the mount's source path exist. `upload_file` is gone; `migrate_data` now *stages* a consistent snapshot next to the running database and returns `next_steps` naming the two files and their destination, and the panel lists them, because an operation that quietly did half the job and reported success is the failure mode worth designing against. Binary transfer goes over SFTP with one of the router's own `/user` accounts - note the REST user (`rest`) is not `admin`, and assuming a login name is how the first attempt got "Permission denied". Lesson: an error message is part of the interface - if the specific reason is in one field and the generic one is read first, every downstream debugging session starts blind, and this feature was designed on paper twice because of it. Any REST shape asserted without a device in the loop should be written as a failing test the first time the device contradicts it, and "the API can send a file" belongs in that category.

**[2026-09-08] Problem:** The container was created with `logging=yes`, so RouterOS copied MikroMan's stdout into its own log ring - and the ring turned out to be MikroMan talking to itself: **995 of its 1000 lines** (`742` `httpx` request lines, `236` uvicorn access lines, `17` other app output) against **5** real device events. Three things followed from that. The ring's turnover dropped to about five minutes, so genuine events were evicted before the 60-second scraper could copy them out - the feature that exists to keep history was destroying the source of it. Every one of those lines was also read back over REST, classified and inserted, then pruned again: ~300 inserts and ~300 deletes a minute on a database of that size on a USB stick. And the "high CPU/memory on the container" reading that prompted the investigation was partly an artefact - `/container` reports `memory-current` as the cgroup's usage, which counts the page cache the app's own writes push through the mount, so it sat near 500 MB while the router's `free-memory` never moved.
**→ Solution:** Per-logger caps, not a global level raise: `httpx`, `httpcore`, `uvicorn.access` and `aiogram` go to WARNING, everything else stays at INFO, because the app's own warnings are the thing that must survive (`backend/app/core/logging_config.py`). That app log now also has somewhere to live that is not the router's RAM - a size-capped `RotatingFileHandler` at `<data dir>/mikroman.log`, resolved from `DATABASE_URL` so the log travels with the database it describes, degrading to console-only when the directory cannot be written instead of failing to start, and readable back through `GET /api/v1/logs?source=app` because a RouterOS container has no `docker logs`. `logging=yes` was deliberately *kept*: with the echo quiet it costs a few lines a day, and it is the only channel that can carry a crash traceback that happens before the file handler exists - turning it off to fix a noise problem would have traded a crash-blind device for a quiet one. Two transferable notes. Measuring this at all was possible only because the echo works both ways: while `logging=yes`, the app's own request lines in the router log are a free per-request trace, and that is how the 741-calls-in-320-seconds figure was produced - the bug was also the instrument. And the same reasoning that caught the memory artefact applies to the CPU figure: ask what the meter counts before arguing about whether the number is too high. Lesson: any log a machine can read is a feedback loop - if the app's output is ingested by the app's own collector, the volume is unbounded and the first casualty is the data the collector was there to preserve.

**[2026-09-08] Problem:** `database is locked` was appearing every few minutes despite WAL, `synchronous=NORMAL` and `busy_timeout=5000` all being verifiably active (`PRAGMA journal_mode` → `wal`, `busy_timeout` → `5000`, and an `app.db-wal` on the mount). The naive diagnosis - "WAL is not configured, add `BEGIN IMMEDIATE`" - was wrong, and the first attempt at a regression test for the read-then-write upgrade deadlock could not reproduce it at all, which is what pointed at the real cause: SQLite allows **one writer**, and some write transaction was holding that lock for longer than five seconds, so the statements in the log were the *victims* (`DELETE FROM system_metrics WHERE timestamp < ?`, the interface-rollup delete for router 2), not the offenders. The offenders were the retention passes: `collect_and_store` ran two unbounded range deletes over the two largest tables **on every 10-second telemetry tick, for every router**, and `prune_old_logs` materialised all 10 000 retained ids in Python to build a giant `IN` list. The measured log-scrape tick slept 60 s and then took ~30 s - most of it queued behind those locks.
**→ Solution:** Bound the transactions instead of scheduling around them: `backend/app/db/prune.py` deletes in committed batches (`LIMIT` inside the subquery, oldest first), which caps how long any one write lock is held, and retention runs hourly rather than per tick (`MetricsCollector._prune_expired`, monotonic deadline so an NTP step can neither skip a pass nor repeat it on every tick) and catches up after downtime because the cutoff is recomputed from the clock, not from a stored cursor. The log cap is now expressed as a timestamp found by walking the index to the `max_records`-th newest row, so no id list exists at all. Alongside: the tick was split so the expensive half (device discovery, queue and mangle reconciliation, rollups, quota checks - ~60 % of all REST traffic, measured 303 of 741 calls) runs every `HEAVY_SYNC_INTERVAL_SECONDS` and is staggered per router so three routers no longer open three long write transactions back to back; `/system/resource` is handed to `collect_and_store` instead of being read twice per tick; and `/ip/cloud` is cached (`58` reads in 320 s had been feeding a 15-minute resolver cache). Shutdown got an explicit per-step budget inside the container's 10-second `stop-time`, which is what `Exited (137)` meant - SIGKILL mid-teardown. Lesson: `busy_timeout` is a wait, not a fix - when it expires, look for the transaction that is too *long*, and remember that a lock is held until `COMMIT`, so any write transaction that spans a network call is measured in router round-trips. And when a suspected root cause cannot be reproduced in a test, treat that as a result: it falsified the theory rather than the test.

**[2026-09-08] Problem:** After the move onto the router, switching the traffic-history presets (today / yesterday / week) took seconds, and a measured profile of the same request on a copy of the live database found the cost was **identical for every preset** - 21 SQL statements either way, so the range was irrelevant and something fixed was dominating. Two causes, neither visible in the code that "looks slow". First: `Device.history` is `lazy="selectin"`, so *any* device load also materialised every event each device ever recorded - one flapping device had **tens of thousands of rows**, and the analytics path never reads that collection. Measured: the same device list with history took **~100x longer** than with `noload()`; the endpoint went from hundreds of milliseconds to single digits. Second: SQLite had **no statistics at all** (no `sqlite_stat1`, nothing had ever run `ANALYZE`) and no composite indexes, so a one-hour interface chart picked `ix_interface_metrics_router_id` and walked **the router's whole index** to return a few dozen buckets; forcing the composite time index cut the visit count by two orders of magnitude. The same statement runs inside the background rollup pass on every tick, so it was also a share of the router CPU the move had apparently "cost".
**→ Solution:** `noload(Device.history)` on the two queries in `get_historical_traffic` (every path that genuinely wants history already names it explicitly with `selectinload`, so the eager load was pure duplication), and composite indexes matching the real shapes - `(router_id, timestamp)` on both metric tables, `(device_id, record_date)`, `(user_id, record_date)`, `(device_id, created_at)` - created by migration `024_query_indexes` **and** at start-up in `db/session.py::_ensure_query_indexes`, because this app has never run Alembic and `create_all` will not add an index to a table that already exists. A test cross-checks the three lists so a name cannot drift. `ANALYZE` runs only when indexes were actually created (~1 s on hundreds of thousands of rows measured - not per restart), with `PRAGMA optimize` otherwise. On the client the same wait had a second, unrelated cause: `loading` was tracked and never rendered, and there was no `AbortController` anywhere, so a preset click queued behind an in-flight 8-second auto-refresh and whichever response arrived last won - the view could end up displaying the range you had just left. Result: two orders of magnitude for the one-hour chart, and the spinner that makes a second of work legible. Lesson: when a change of range does not change the cost, stop optimising the query you suspect and profile the request; and an ORM relationship marked eager is a per-query decision that shows up as a fixed cost in every unrelated reader of the same table.

**[2026-09-08] Problem:** Two things that had been built for one migration onto the router stayed in the product afterwards. `POST /containers/migrate-data` - snapshot the live database, stage the key, tell the operator to SFTP them - is a one-time cutover operation, and as a permanent button it invites someone to replace a running deployment's database. Separately, the container setup judged storage by asking `/file` whether a path with that name existed, which answers yes to a stick mounted read-only, to an NTFS volume RouterOS cannot write, and to a 128 MB partition that cannot hold a 340 MB image - all of which then failed mid-pull with a registry error instead of a reason.
**→ Solution:** The migration feature is deleted (service block, schemas, route, client method, panel button, tests, docs), and storage is judged from `/disk`, which reports `fs`, `mounted`, `mount-read-only`, `size`, `free` and `formatting` per device and partition: `assess_storage()` returns blocking problems and non-blocking warnings, `GET /containers/storage` feeds a picker built from what exists rather than a text box, and formatting is offered - guarded: the slot has to be retyped into `confirm`, it must be a row the device reports, the filesystem must be one RouterOS writes, and the target (or its parent, or any partition of it) must hold no `layer-dir`, `tmpdir` or mount source, which is also what stops the app wiping the storage it booted from. Lesson: a one-time migration tool is a destructive button with a story attached; when the story stops being true, delete the button. And prefer the menu that models the concept - RouterOS has a storage subsystem, and `/file` is a directory listing that cannot distinguish "present" from "usable".


**[2026-09-08] Problem:** The eager `Device.history` load that dominated the analytics request had a cause behind it: one device row carried **tens of thousands** history rows after six days — 17 561 `ip_changed` and 17 561 `hostname_changed`, spread evenly over exactly four `(address, hostname)` pairs. Two hosts were answering with **the same MAC** on two subnets (192.0.2.11 `WIN-HOST-A`, 192.0.2.12 `WIN-HOST-B`); the discovery loop iterates leases, so it visited that MAC twice per sweep and each visit "changed" the IP and the hostname away from what the other had just written. Four rows per sweep, ~10 800 a day, forever — `device_history` had no retention of any kind and is cleared only when a router row is purged.
**→ Solution:** Collapse the input before the loop: `_one_lease_per_mac()` keeps one lease per MAC, chosen **deterministically** (`bound` first, then address ascending), so the winner cannot itself alternate between two permanently-present hosts — "the newest lease" would have re-created the flip-flop at half the rate. The condition is reported once per MAC per process, not once per sweep: two hosts on one MAC means the "device" is really two, and that deserves to be said once a day, not 1 440 times. Plus `prune_device_history()` on the existing 6-hour maintenance tick (90 days, batched). Two smaller bugs the new tests caught: `delete_older_than()` reached for a `timestamp` column that `device_history` names `created_at` — a `KeyError` inside a background prune nobody watches — so the time column is a parameter now; and the cutoff compared **local** `datetime.now()` against a column whose `func.now()` default writes **UTC**, silently keeping or dropping a window equal to the host offset (+05 here), so it compares naive-UTC like `metrics_collector` does. Lesson: a table growing 10 000 rows a day is a *writer* bug, not a retention problem — find who writes and why they believe the value changed before adding a purge; and when the fix is "pick one", the picking rule must be stable across runs, not merely sensible within one.


**[2026-09-09] Problem:** After the previous round shipped, the container's memory kept climbing under RouterOS' own eyes: 508 MB and a 38 % CPU peak in the Winbox Containers tab, and `/diagnostics` showed the process RSS walking 347 MB → 398 MB (peak 417) across 98 minutes with no load change. Measuring the background work over that window: `sync.heavy` occupied 33 % of wall time, `log_scrape.tick` 26 %, `sync.telemetry` 13 % — about 72 % of a single event loop, against 3.35 REST calls/second. The memory curve was not a leak in the app's own structures: `Device.history` is eager (`lazy="selectin"`) by relationship, so **the discovery sweep's `select(Device)` materialised every event row of every device once a minute** — the tens-of-thousands-row device from the previous entry included, roughly 50 MB of ORM objects built and discarded per minute. CPython returns freed arenas to its own allocator, not to the OS, so the resident set of a long-running process only ratchets upward: a sawtooth demand curve that reads as a leak.
**→ Solution:** `noload(Device.history)` on the discovery query (nothing in it reads history; every path that does already `selectinload`s explicitly), plus `cap_device_history()` keeping the newest 200 events per device — because the age-based retention added an hour earlier **could not have fixed an installed system**: those rows are six days old, so a 90-day rule would keep them until spring. Two things to carry forward. First, an eager relationship is not a property of the code that reads it — it is a cost imposed on every query that touches the parent, so when a load turns up expensive, enumerate the sweeps and background loops, not just the endpoints. Second, the release before this one made the peaks *higher* and that was mine, not a mystery: running the routers concurrently did not reduce the work, it let the same work overlap, so total busy time held steady while instantaneous CPU rose. Parallelism is a latency fix; only removing work is a load fix, and the honesty check on a "it got worse" report is to measure duty cycle before defending the change.

**[2026-09-09] Problem:** The fix above shipped, and the container's resident set still climbed within the first seven minutes, with the high-water mark a few tens of megabytes above it. The previous entry's own conclusion ("enumerate the sweeps, not just the endpoints") had been applied to exactly one sweep. `find_merge_suggestions()`, which runs on **every heavy pass**, still eager-loaded `Device.history` through the same relationship, so the process was still building and dropping ~18 MB of ORM objects a minute. Removing the explicit `selectinload(Device.history)` from that query changed nothing at all, which is what proved the point: the eager load comes from the **relationship default**, not from the call site, so omitting the option is a no-op and only `noload()` suppresses it. The regression test written for the first sweep failed against the second file and said so in SQL (`SELECT device_history.device_id …`), which is how the second offender was located rather than guessed.
**→ Solution:** `noload(Device.history)` on both queries in `find_merge_suggestions()` (`selectinload(Device.user)` stays — the suggestion label needs the owner's name), and a start-up pass in `lifespan` that runs the age prune and the per-device cap instead of waiting up to eight hours for the first maintenance tick, since the process that has to survive the accumulated rows is the one that pays for them. Two general notes. First: an ORM default that costs ~18 MB per query cannot be fixed by *not* opting in — search for every reader of the parent table, and let a test that asserts the child table is never touched be the thing that enumerates them. Second: bounded retention expressed only in age is not a bound. The tens-of-thousands-row device was six days old, so a 90-day rule kept every row of it; `HISTORY_KEEP_PER_DEVICE` is the guarantee, the age rule is hygiene. Lesson: "the same bug in a second place" is the most likely explanation for a fix that measurably did nothing, and a guard test that fails loudly on the untouched path is cheaper than another release cycle.

**[2026-09-09] Problem:** After the memory fixes landed, the container still spent **69 %** of one core on background work, and the single biggest line in the profile was `log_scrape.tick`: **avg 17.5 s, max 21.3 s of a 60 s cycle** (an earlier window caught it at 40.5 s). The tick was doing three unrelated jobs — copy the router's log ring, fold conntrack plus the DNS cache into the destination history, and audit exposed management ports — all at the log ring's required pace. The proposal that had been written a day earlier was wrong about which half to cut: it listed the security audit as a target, but the audit is *one* small request per router, while the destination sample is two large ones. That was only caught because each sub-pass got its own timer.
**→ Solution:** Gate by job, not by loop: log copy stays every tick (`LOG_SCRAPE_INTERVAL_SECONDS` unchanged — the ring turns over in minutes and everything older is gone), destinations every 3rd tick, audit every 5th, through one `_tick_due(tick, every)` helper gated on the 0-based count so the first tick after start-up still populates both panels. `DESTINATION_SAMPLE_EVERY_TICKS = 1` is the documented way to undo a gate, which is why the helper handles `every <= 1`. Counting ticks rather than watching a clock is deliberate: the tick carries its own work, so a gated job's real period stretches when the machine is loaded — spending less per unit time under load is the point, not fighting the machine for a fixed period. Accepted cost, stated plainly: a connection that opens and closes between samples is undercounted in the destination tab (the delta accounting itself stays correct, and conntrack's `time-wait` linger covers more short flows than expected). Two transferable rules. Gate work by *whose* freshness contract it serves, and when a tick looks expensive, time its parts before proposing what to cut — the plausible-looking candidate was the cheap one.

**[2026-09-09] Problem:** A test that asserted a masked Telegram token renders its "hidden" hint passed alone and failed in a full run. The component was not broken and the assertion was not wrong: `src/test/setup.js` calls `vi.restoreAllMocks()` after every test, which strips the implementations that the file's `vi.mock('../api/client')` factory handed out. A later test that re-stubs *one* method — here `getSettings` — leaves `getRouters()` returning `undefined`, and `Promise.all` then throws inside the modal's open effect, where the component's own `try/catch` logs it and falls through to defaults. Two traps stacked: a failure that looks like a UI bug is a fixture-state bug, and because the component swallows the error, the symptom is silence rather than a stack. Related, same cause: a contract constant first exported from `api/client.js` was `undefined` under those same wholesale mocks, and the component threw on *render*.
**→ Solution:** In that describe, a `withSettings()` helper re-establishes the entire fan-out the modal performs on open, not just the response under test; and the placeholder both sides of the settings API agree on moved to `src/api/secrets.js`, a module no test replaces, with `client.js` left as transport only. Rules worth keeping: when a test passes alone and fails in a suite, suspect shared mock state before the component, and never assert on a code path whose errors the unit under test catches; anything imported by a module that tests mock wholesale is unavailable in those tests, so contract constants belong in a module nobody swaps out.

**[2026-09-09] Problem:** While verifying the settings-mask round-trip on the live device, the app log turned out to be a wall of `TelegramConflictError: Conflict: terminated by other getUpdates request; make sure that only one…` — 122 occurrences in the 11 minutes the rotating file still showed, one per second, still running 18 minutes later on a container that had booted exactly once. Cause: `reconfigure()` (the path every Settings save takes) called `stop()`, which cancelled the task wrapping `dp.start_polling(bot)`, then `_init_bot()` assigned a **new** `Bot`/`Dispatcher` to `self`. Cancelling the wrapper does not tear down the HTTPS session underneath, and dropping the last reference to the old `Bot` never closes it — so Telegram kept the old session's `getUpdates` slot open and refused the new one, forever. Not a v0.2.9 regression: any operator pressing Save with a token configured reproduced it, before and after. My POST just walked into a bug that had been sitting on the save path.
**→ Solution:** `stop()` now stops polling through aiogram's own `dp.stop_polling()` (bounded by `asyncio.wait_for`, since the container gets ~10 s before SIGKILL), cancels the task only if it is still alive, and **closes the bot session**. `start()` additionally refuses to open a second session while one is live, because two pollers on one token is an outage, not redundancy. Five tests in `tests/test_telegram_lifecycle.py`, the important one asserting `live == 1` polling session and `old_bot.closed is True` after a `reconfigure()`. Three things to carry forward. First: *replacing an object that owns a connection is a leak* — assignment is not teardown; if a method swaps the client, it must close the one it swapped out. Second: a background task that logs an error every second is a self-inflicted log flood on a device whose log is ours to read — a permanent-retry loop deserves a backoff and a "reported this already" guard as much as a fix. Third, the meta-lesson: the mask change was verified by *exercising the real save path*, and that is what found it; a unit-test-green release deployed onto a device is still only a hypothesis until something touches it end to end.

**[2026-09-09] Problem:** Opening the monitoring page visibly raised the **router's** CPU, rhythmically, while the app's own accounting said background work was ~52 % of the loop. The gap was structural: every recorded pass lived in the background workers, and the per-second WebSocket telemetry frame was timed by nothing at all — so the one cost that scales with "is anyone looking" had no number next to it. Inside that frame were two independent mistakes. `get_realtime_traffic_stats` ran plain `select(User)` and `select(Device)` to render a frame of scalar columns, and because `User.devices`, `User.traffic_rollups`, `Device.history`, `Device.traffic_rollups`, `Device.linked_adapters` and `Device.primary_device` are all `lazy="selectin"`, each frame cascaded into the event log — the fourth instance of a bug already fixed in analytics, discovery and consolidation. The same frame called `/ip/firewall/mangle` with no `.proplist`, making RouterOS render every attribute of every accounting rule once a second per browser; the rate tracker reads `comment` and `bytes` and nothing else. A third of the cost was self-inflicted: the hook reconnected on `onclose` without checking visibility or its own readyState, so a hidden tab kept polling forever and a page could end up holding two sockets.
**→ Solution:** `.proplist` on the hot read via `get_mangle_rules(fields=...)`, leaving the reconcilers' unqualified print intact because they need the attributes they write; `noload()` on every relationship the frame does not read; `record("ws.telemetry_tick", …)` so this path can never again be invisible in diagnostics; and in the hook, close on `visibilitychange` to hidden, suppress the reconnect while hidden, and refuse a second socket while one is CONNECTING or OPEN. Three transferable points. A page that is *only* a viewer is still a client with a polling rate — instrument the request path that exists because a human is watching, not just the schedulers. An eager relationship is charged per query, so the fix has to enumerate readers, and a guard test that asserts on executed SQL is what enumerates them. And a test double whose signature is narrower than the real object's is a silent failure generator: the added `fields` kwarg made three fakes raise `TypeError`, which the code's own `except Exception` swallowed into zeroed rates — the test that caught it looked like a product regression.

**[2026-09-11] Problem:** On quad-core ARM64 RouterOS gateways, `python3.12` consistently pegged exactly one CPU core at 100% (25.8% total router CPU) while the router REST API (`www`) sat at only 0.1% and no browser was open. Two runtime defects caused this single-thread CPU spin: (1) `uvloop` on Python 3.12 under Linux `aarch64` entered an internal `epoll` busy-polling spin loop rather than yielding idle sleep to the kernel; (2) In `v0.2.13`, connection pooling (`pool_size=5, max_overflow=5`) was added to `create_async_engine` for SQLite with `aiosqlite`, instantiating SQLAlchemy's `AsyncAdaptedQueuePool` where background worker threads spun on futex locks while checking connection health.
**→ Solution:** Configured Uvicorn CMD in `Dockerfile` to explicitly use `--loop asyncio` across all container platforms, bypassing `uvloop`'s ARM64 busy-wait bug and utilizing Python 3.12's optimized standard event loop. Replaced `QueuePool` in `backend/app/db/session.py` with `NullPool` for persistent SQLite connections, ensuring each async session creates and disposes of `aiosqlite` worker threads cleanly with zero lock spinning or thread contention. Lesson: An idle background daemon should consume <0.5% CPU; when a core pegs at 100% while upstream API load is near zero, never attempt to parallelize the workload across more cores (which would lock up all 4 cores at 100%). Find and eliminate the spin loop in the event loop or thread pool.

**[2026-09-11] Problem:** Live bandwidth counters showed frozen zeroes (`↓ 0 bps / ↑ 0 bps`), the ISP billing cycle reset to Day 1 on page reload, and the historical traffic usage chart was empty (`Timeline (0 days)`) after switching to the Go backend core. Three underlying defects caused this: (1) The Go backend never created RouterOS mangle passthrough counter rules (`mikroman:acct:dev_<id>:{up,down}` and `mikroman:acct:self:{up,down}:<wan>`), leaving RouterOS with no byte counters for devices, while the telemetry loop hardcoded device/user rates to 0; (2) The Go HTTP router lacked the `/api/v1/analytics/billing-cycle` (GET & POST) endpoints and only read `isp_billing_cycle_anchor_day`, while the frontend and settings stored `billing_cycle_anchor_day_<routerID>`; (3) `/api/v1/analytics/traffic` returned a flat dummy payload lacking the `data` wrapper, daily timeline points, gateway totals, interface summaries, and accounting health.
**→ Solution:** Implemented `SyncCounterRules` in the Go traffic service to automatically provision, reconcile, and prune mangle accounting rules and accumulate 15-minute and daily rollups; in telemetry service, queried live interface rates via `/interface/monitor-traffic` and calculated per-device/per-user rates from mangle byte deltas; registered `/analytics/billing-cycle` on the Go router with active-router resolution and persistence; and fully implemented `/analytics/traffic` returning calendar-aligned daily timeline points, gateway aggregates, user/device breakdowns, and accounting coverage health.

**[2026-09-11] Problem:** Live bandwidth counters remained at or near zero (`↓ 0 bps / ↑ 0 bps`) on the dashboard despite traffic activity, while the "Router Health" charts (interface traffic, CPU load, RAM usage) and the "Live Connections" tab failed to load with HTTP 404 errors. Three structural bugs were uncovered: (1) The Go background telemetry loop broadcasted telemetry ticks across all active routers in the database to all connected WebSocket clients indiscriminately; because frames for Router 1 (with traffic) were followed in the same millisecond by empty frames for secondary routers (0 bps), the browser's view was continually clobbered by zeroed frames; (2) In `TelemetryService`, rate tracking baselines (`prevDeviceBytes`, `prevIfaces`, `prevTime`) were shared singleton maps across all routers, so an iteration for an idle secondary router wiped the previous sample baseline of the primary router, permanently preventing delta rate division `(cur - prev) / dt`; (3) The Go HTTP router lacked implementations for `/api/v1/metrics/system`, `/api/v1/metrics/interfaces`, and `/api/v1/connections`.
**→ Solution:** Scoped WebSocket client subscriptions by router ID (`?router_id=X`), protected connections with a send mutex, and cached the latest router frame for immediate playback upon client connection. Replaced shared singleton rate baselines in `TelemetryService` with router-scoped maps (`map[int]...`), preventing cross-router baseline stomping. Added flexible JSON unmarshaling (`FlexibleInt64`) for RouterOS firewall rule byte counters and mangle comment parsing handling both 4-segment and 5-segment comments. Implemented downsampled peak-preserving system and interface metrics endpoints (`/api/v1/metrics/system` and `/api/v1/metrics/interfaces`) with 1h/6h/24h/7d/30d bucket aggregation, and implemented live firewall connection tracking with local/external endpoint classification and remote connection termination (`/api/v1/connections`).
**[2026-09-12] Problem:** Updating to v0.3.14 caused the backend API to freeze and stop responding on startup: all dashboard telemetry dropped to 0 bps, status tiles became inactive, and REST endpoints timed out or failed with HTTP 500 `database is locked`. Three distinct bottlenecks combined:
1. SQLite database lock starvation on startup: `TelemetryService.backfillInterfaceRollups` executed an unindexed `SELECT ... FROM interface_metrics GROUP BY router_id, interface_name, DATE(timestamp)` query on startup. Over large historical tables, this aggregation held SQLite's exclusive write lock for minutes, easily exceeding `busy_timeout=5000` and causing all incoming API requests (`/api/v1/auth/status`, `/api/v1/routers`, `/api/v1/users`) to fail with 500 errors and preventing the telemetry loop from running.
2. RouterOS REST API flooding via conntrack flushes: `ReconcileQueues` called `client.FlushConnectionsForIP` in a loop for every queued IP on startup and during heavy sync passes. For each IP, it downloaded the conntrack table and issued individual REST DELETE requests for every active socket. RouterOS's lightweight embedded `www` server was overwhelmed by hundreds of rapid HTTP calls, causing timeouts and socket disconnects that starved the telemetry poller's `GetSystemResource` and interface monitors.
3. Mutex contention across network I/O: `TelemetryService` held its synchronization mutex across external RouterOS network calls and database writes, blocking concurrent readers (`GetLatestRates`, `GetLatestFrame`).
**→ Solution:** Removed the synchronous startup backfill (`backfillInterfaceRollups`); rollups are accumulated incrementally during regular telemetry polling, and live fallback queries handle missing rollups on demand. Removed connection flushing on queue sync, relying on RouterOS simple queues and FastTrack exemption to govern active streams without restarting sockets. Narrowed the telemetry mutex critical section to protect only in-memory state snapshots, releasing immediately before external network and database I/O. Lesson: In single-writer embedded databases, never run synchronous aggregation backfills on process startup. Furthermore, never issue loops of per-socket HTTP mutations against an embedded router web server; let native kernel queueing apply to existing streams naturally.

**[2026-09-12] Problem:** Opening the World Map modal in the Live Connections tab triggered an immediate application crash with `Minified React error #310` ("Rendered more hooks than during the previous render").
**→ Solution:** `if (!isOpen) return null;` was placed before two `useCallback` hooks and one `useEffect` hook in `WorldConnectionsModal.jsx`. When the modal was toggled open, React executed more hooks than during the closed state, violating the invariant call order required by the Rules of Hooks. Moved the early return guard after all hook definitions immediately prior to the JSX output. Lesson: Never place early conditional returns before React hooks; all hooks must execute in an identical sequence on every render regardless of modal visibility.

**[2026-09-12] Problem:** In the Unassigned Devices tab, checking "Show Hidden" failed to display hidden devices, leaving them inaccessible.
**→ Solution:** `loadData` in `App.jsx` initially passed `showHiddenDevices` (default `false`) when querying unassigned devices, preventing hidden devices from being fetched into client memory. Updated `loadData` to fetch all unassigned devices (`show_hidden=true`), allowing the reactive filter in `DeviceInbox` to instantly toggle visibility between visible and hidden items without extra network latency or missing records. Lesson: When a client-side component manages toggleable visibility over a dataset, the initial fetch must supply the complete set rather than pre-filtering on the server with an initial negative state.

**[2026-09-12] Problem:** Historical traffic timeline bar charts in `/analytics/traffic` displayed zero usage for every day, even when active traffic and interface counters existed.
**→ Solution:** The Go analytics handler only queried `router_traffic_rollups` (a legacy table; new rollups are stored in `interface_traffic_rollups`). Furthermore, SQLite date matching failed when rollup timestamps included time components (e.g. `YYYY-MM-DD HH:MM:SS` or ISO8601 strings), and device rollups were not scoped to the active router. Updated the timeline query to aggregate `interface_traffic_rollups` for WAN interfaces, legacy rollups, and router-scoped device rollups, using `DATE(record_date)` and 10-character string slicing (`dStr[:10]`) for consistent calendar-day grouping. Lesson: When migrating schema tables and relying on string-formatted dates across database drivers, always normalize date keys to the exact ISO `YYYY-MM-DD` boundary and query the active rollup tables.

**[2026-09-12] Problem:** Telemetry polling updates appeared approximately twice as slow as the configured interval (e.g., updating every ~4 seconds despite a 2-second setting).
**→ Solution:** `StartBackgroundLoop` slept for `time.After(interval)` *after* the collection cycle finished. Because `Collect()` required ~1.5–2.5 seconds due to sequential REST calls, total cycle time became `duration(Collect) + interval`. Switched loop timing to calculate remaining sleep `sleepDuration = interval - time.Since(start)` with a minimum 50ms guard. Added caching for static/infrequently changing RouterOS endpoints (RouterBOARD info 10m TTL, IP addresses 30s TTL, Cloud DDNS IP 60s TTL) to cut redundant REST overhead per cycle. Lesson: In periodic polling engines, never sleep for the full interval after completing work; subtract elapsed execution time to ensure steady cadence.

**[2026-09-13] Problem:** Router log was flooded every 60 seconds with `/queue simple set` configuration updates for every user and device queue during the periodic background sync. `ReconcileQueues` intended to only update modified queues, but direct string comparisons permanently evaluated to true because: (1) RouterOS expands rate limits to raw bps (e.g. `11000000/11000000`) whereas the application database stored shorthand units (`11M/11M`); (2) RouterOS reports `parent="none"` for root queues whereas the application expected `""`; (3) RouterOS appends `/32` subnet notation to host targets and may reorder multiple IP targets.
**→ Solution:** Introduced canonical normalizers (`normalizeRateLimit`, `normalizeTarget`, `normalizeParent`) in `TrafficService`. Rate limit tokens (with `k`, `m`, `g` suffixes) are parsed into bits-per-second integer strings, parent values `"none"` and `""` are treated as equivalent, and target IP lists are normalized to `/32` notation and sorted before comparison. If the desired configuration matches the active RouterOS queue state, mutation commands are bypassed completely, eliminating router log spam.

**[2026-09-13] Problem:** Setting the telemetry streaming interval to 1 second produced updates every 1.5–2.0 seconds rather than every 1.0 second. Profiling revealed `Collect()` itself took ~1.4–1.8 seconds per execution due to: (1) 5 sequential REST calls per router; (2) fetching full mangle rule tables without `.proplist`; (3) heavy billing cycle table aggregations on every tick; (4) individual unbatched SQLite metric inserts per interface every second.
**→ Solution:** Dispatched independent RouterOS REST calls (`/system/resource`, `/interface/monitor-traffic`, `/ip/firewall/mangle`, `/system/health`) concurrently using `sync.WaitGroup`; queried mangle rules with `.proplist=.id,comment,bytes`; cached system health and billing volume statistics for 5 seconds; decoupled historical metrics and interface rollups to commit in a single SQLite transaction on a 10-second cadence; and polled multiple active routers concurrently in `StartBackgroundLoop`. Execution latency of `Collect()` dropped to <100ms, achieving true 1.0-second broadcast cadence.

**[2026-09-13] Problem:** Live WAN sparkline charts and bandwidth rate tiles in the top navigation bar were permanently flat at `0 bps`. RouterOS 7 REST API returns instantaneous rate metrics from `POST /interface/monitor-traffic` as JSON strings (e.g. `"rx-bits-per-second": "15000000"`). Standard Go `float64` unmarshaling failed silently with `json: cannot unmarshal string into Go struct field of type float64`, leaving the rates array empty and calculating 0 bps.
**→ Solution:** Implemented `FlexibleFloat64` custom unmarshaler supporting both JSON strings and numeric float representations. Updated `InterfaceTrafficRate` to use `FlexibleFloat64`, and added fallback computation of WAN rates from interface byte counter deltas if monitor-traffic returns empty results.

**[2026-09-13] Problem:** In the Unassigned Devices tab, two device cards for the same physical device (e.g. dual-band Wi-Fi or roamed interface) displayed the same IP address with both marked with a green `ACTIVE` badge. The discovery service set `is_active = 1` for detected MACs, but never inactivated devices that were no longer present in DHCP leases, ARP table, or Wi-Fi registrations, causing disconnected or band-swapped MACs to retain active status and stale IPs indefinitely.
**→ Solution:** In `DiscoveryService.SyncDevices`, after processing present devices, queried all existing devices for the active router and set `is_active = 0` for any MAC not found in the current sweep. In addition, when an active device claims an IP address, any other device record on that router previously holding that IP has its stale `ip_address` set to `NULL`.

**[2026-09-13] Problem:** Switching routers in the top selector to a secondary router (e.g. `Remote Router`) continued displaying live firewall connections and interface lists from the primary default router. In `ConnectionsHandler`, `getClient` checked `if defaultRouter.ID == targetID && h.client != nil`. When the secondary router was activated as default in the database, `defaultRouter.ID == targetID` evaluated to true, returning the startup singleton client `h.client` (which was permanently bound to the primary router's host/port). Furthermore, `GetInterfaces` in `SystemHandler` and `ListAvailableInterfaces` in `MetricsHandler` ignored the `router_id` query parameter and always queried `h.client`.
**→ Solution:** Replaced naive default router identity comparisons with explicit host and port validation `client.Matches(router.Host, router.Port)`. If the existing client does not match the target router's connection parameters, a dedicated `routeros.Client` is instantiated and cached per router ID. Added `router_id` query parameter support and client resolution to `GetInterfaces` and `ListAvailableInterfaces`. In the frontend, added router-keyed component remounting (`key={activeRouter?.id}`) to `LiveConnectionsModal` and `MetricCharts`, and reset interfaces state on router switch.

**[2026-09-13] Problem:** Running speed test caused WAN upload to spike briefly, but download was never measured (leaving download rate at 0 and WAN download flat) and no speed figures were displayed in the UI badge. Investigation revealed: (1) `measureDownload` appended `?bytes=15000000` to the Cloudflare speed test endpoint, which Cloudflare rejected with HTTP 403 Forbidden because only standard chunk sizes (25MB, 10MB) are accepted; (2) `measureDownload` did not check HTTP status code 200, terminating immediately and returning `download_mbps = nil`; (3) in `SpeedTestBadge`, the UI logic strictly required `download_mbps != null` to render any speeds, hiding valid upload results.
**→ Solution:** Updated `measureDownload` to use standard 25MB chunks (`?bytes=25000000`) with dynamic 10MB fallback on non-200 responses, implemented looping worker streams for sustained accuracy over the test duration, added explicit `resp.StatusCode == http.StatusOK` checks on both download and upload, and updated `SpeedTestBadge` to display speed figures whenever either download or upload measurements are present.

**[2026-09-13] Problem:** Speed test upload figures were dropped (`↑—`) when slower connections timed out before full multi-megabyte POST requests completed, and the badge button text overflowed the WAN IP header tile ("Speed tes"). In addition, the World Connections map suffered from four UX limitations: (1) multiple overlapping nodes in dense regions (such as Europe) created illegible visual clouds; (2) mouse-wheel zooming failed because the DOM listener was attached before the modal opened; (3) the bottom connection list was an excessively long single-row strip; (4) only ~30 countries had vector flags, leaving many countries (e.g. South Africa `ZA`, Cyprus `CY`) with raw fallback code boxes.
**→ Solution:** (1) Implemented `countingUploadReader` in `measureUpload` with smaller 256 KB POST chunks, accumulating transmitted bytes in real-time as bytes leave the socket buffer so partial streams are accurately recorded even on timeout or cancellation; rendered a compact icon-only gauge button in `SpeedTestBadge` once figures are present, preventing label overflow. (2) Introduced dynamic visual distance-based node clustering in `WorldConnectionsModal`: nearby countries automatically merge into multi-country cluster nodes at low zoom with aggregate connection counts, and smoothly split into individual country nodes as the operator zooms in; attached a non-passive wheel listener on modal open with layout-independent fallback coordinates for instant mouse-wheel zooming; converted the bottom country list into an auto-flowing 5-rows-per-column horizontally scrollable grid table. (3) Replaced hardcoded switch statements in `CountryFlag` with locally bundled vector SVGs for all 265 ISO-3166-1 alpha-2 territories via `country-flag-icons` and `Intl.DisplayNames`, preserving zero external CDN dependencies for air-gapped router deployments.

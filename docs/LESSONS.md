...
**[2026-09-08] Problem:** The eager `Device.history` load that dominated the analytics request had a cause behind it: one device row carried **35 123** history rows after six days — 17 561 `ip_changed` and 17 561 `hostname_changed`, spread evenly over exactly four `(address, hostname)` pairs. Two hosts were answering with **the same MAC** on two subnets (172.16.141.254 `WIN-R1I13RGSAUB`, 172.16.142.254 `WIN-41JDL2PAM9Q`); the discovery loop iterates leases, so it visited that MAC twice per sweep and each visit "changed" the IP and the hostname away from what the other had just written. Four rows per sweep, ~10 800 a day, forever — `device_history` had no retention of any kind and is cleared only when a router row is purged.

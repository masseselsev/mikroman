...
async def store_router_offset(session: AsyncSession, offset_minutes: Optional[int]) -> None:
    """Persist the router's UTC offset so date keys survive a restart.

    Written whenever the router clock is read; without it a freshly started
    container would file traffic against UTC dates until the first poll.
    """
...
async def get_router_offset(session: AsyncSession) -> Optional[int]:
    """The router's stored UTC offset in minutes, or None if not yet known."""
    setting = await session.get(AppSetting, ROUTER_OFFSET_SETTING_KEY)
    ...
    try:
        ...
        logger.warning("Stored router UTC offset is not a number; falling back to UTC")
        return None


async def router_local_date(session: AsyncSession, now_utc: Optional[datetime] = None) -> date:
    ...
    return shift_to_router_local(moment, await get_router_offset(session)).date()


async def router_local_now(session: AsyncSession) -> datetime:
    """Current wall-clock time on the router."""
    moment = datetime.now(timezone.utc).replace(tzinfo=None)
    return shift_to_router_local(moment, await get_router_offset(session))

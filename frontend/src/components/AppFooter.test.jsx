...
describe('AppFooter', () => {
  ...
  it('renders glowing update badge when a newer release is detected', async () => {
    api.checkAppVersion.mockResolvedValue({
      data: {
        ...
        latest_version: '0.3.33',
        has_update: true,
        release_url: 'https://github.com/masseselsev/mikroman/releases/tag/v0.3.33',
      },
    });
    ...
    expect(badge.getAttribute('href')).toBe('https://github.com/masseselsev/mikroman/releases/tag/v0.3.33');
    expect(badge.textContent).toContain('0.3.33');
  });
  ...
  it('drops the offer once the running build catches up with the latest release', async () => {
    api.checkAppVersion.mockResolvedValue({
      data: { current_version: '0.3.32', latest_version: '0.3.33', has_update: true },
    });
    ...
    api.checkAppVersion.mockResolvedValue({
      data: { current_version: '0.3.33', latest_version: '0.3.33', has_update: false },
    });

...
* **🧭 Bounded Footprint and Self-Diagnostics**:
  ...
  * **Pure Go High-Performance Core**: Statically compiled binary (`CGO_ENABLED=0`) with modern pure-Go SQLite engine (`modernc.org/sqlite`). Consumes ~4 MB RAM and 0.00% idle CPU, eliminating all interpreter overhead, asyncio futex spinning, and Python memory fragmentation on ARM/MIPS/x86 gateways. Features live bandwidth rate telemetry (`/interface/monitor-traffic` and per-device mangle delta rates), router-scoped WebSocket streams with instantaneous frame playback on connect, ISP billing cycle management (`GET`/`POST /api/v1/analytics/billing-cycle`), full historical traffic analytics (`GET /api/v1/analytics/traffic`) with daily timelines, peak-preserving system and interface metrics (`GET /api/v1/metrics/{system,interfaces}`), and real-time live firewall connection tracking with local/external classification and remote socket termination (`GET /api/v1/connections`, `POST /api/v1/connections/{id}/kill`). Multi-stage Docker builds produce a minimal Alpine container under 30 MB (9.7 MB compressed).

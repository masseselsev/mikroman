...
* **🧭 Bounded Footprint and Self-Diagnostics**:
  ...
  * Device history is bounded at both ends: discovery keeps one DHCP lease per MAC (two hosts answering with the same MAC made it record two "changes" every sweep — 35 123 rows in six days on one device, which every device read then paid for), reports a duplicate MAC once rather than 1 440 times a day, and prunes event rows older than 90 days on the maintenance tick.

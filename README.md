...
* **🔍 Device Discovery, Auto-Scan Toggle & Hidden Devices:**
  * **⚡ Auto-Sort Devices by Activity**: Toolbar toggle on the Users tab that automatically sorts devices within each user card by real-time activity — highest moving throughput first, followed by active status, and then total all-time downloaded volume.
  ...
  * **WAN is resolved from the routing table, not the name**: the Gateway Interfaces modal's **WAN Only** button selected any interface whose name matched `ether1|wan|pppoe|sfp`, which is wrong on a box whose uplink is a VLAN, a second port, or a renamed interface. `GET /api/v1/metrics/interfaces/list` now flags each interface `is_wan` from `/ip/route` — the interface(s) carrying a live default route, read from the resolved next hop (`<gw>%<iface>`, or a bare interface name for PPPoE) — and **WAN Only** selects exactly those, falling back to the name guess only when no default route resolves. Multi-WAN returns every default-route interface. The selection stays a free multi-select, so a split uplink such as `ether1` **+** `vlan500` is two ticks; each flagged interface shows a **WAN** badge, and the Download/Upload tiles list the monitored set with a WAN marker and the full list in their tooltip.

...
* **🧭 Bounded Footprint and Self-Diagnostics**:
  ...
  * Low-memory runtime and live telemetry hot-path optimizations: Strict SQLite page cache bounds (`PRAGMA cache_size = -2000` / 2 MB max, `mmap_size = 0`), bounded SQLAlchemy connection pools (5 connections max), periodic glibc heap compaction via `malloc_trim(0)`, and in-memory TTL caching for device-to-user attribution maps to completely eliminate database hits during high-frequency live WebSocket telemetry frames.

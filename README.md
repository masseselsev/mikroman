...
* **🌐 Real-Time Observability, GeoIP World Map & Centralized Logs**:
  ...
  * Dual-mode **Speed Test Runner**: pure Go in-process engine executing directly when MikroMan is deployed inside a RouterOS container (zero external container or CLI dependencies) with custom fallback DNS resolution and CDN user-agent resilience, transparent fallback to the on-router container runner (`quay.io/tangent/speedtest-cli:latest`) when running externally, and full SQLite metrics persistence.

...
* **📊 Historical Traffic Accounting & ISP Billing Cycles:**
  ...
  * **📈 Interactive User & Device Traffic History Modals**: Dedicated graph modal on every user card and device row across *By Users* and *Unassigned Devices*. View traffic consumption trends across **Day (1D)**, **Week (7D)**, **Month (30D)**, **Year (1Y)**, **All Time**, and **Custom Date Ranges** with interactive stacked download/upload bar charts, summary metrics (total volume, daily average, peak activity date), and per-device breakdown lists.
    * **1D is half-hourly**: the Day view for a user resolves to 30-minute buckets (`HH:MM` axis) instead of a single flat bar, so you can see *when* in the day the traffic happened. It is fed by `user_traffic_buckets` — the same accounted deltas the daily rollup gets, written per user per 30-minute window of router-local time and pruned after 14 days; the daily rollups stay the permanent record. The response carries `resolution: "day" | "half_hour"` and each point an optional `label`; a peak in the 1D view is reported as `peak_label` (`HH:MM`).
...
* **🔍 Device Discovery, Auto-Scan Toggle & Hidden Devices:**
  ...
  * **The WAN set is never auto-selected**: a newly added router starts with **nothing** monitored. The `is_wan` flag from the routing table only *hints* which rows to tick — it is not applied for you. Until the operator saves a selection, the two bandwidth tiles show a loud amber **"⚠ No WAN selected — tap to set"**, the WAN IP tile reads `—`, and **no accounting runs for that router**: `resolve_monitored_interfaces` returns `[]`, so the gateway and per-interface rollups, the billing-cycle boundary slices, and the `mikroman:acct:self:*` passthrough rules are all withheld until an uplink is chosen. Per-device counters are unaffected (they key off the device IP, not an interface).
...
* **🛡️ Multi-Router Management & Automated SSL Provisioning:**
  * **Complete Multi-Router Environment Isolation**: Each managed router operates as an independent environment with its own isolated user profiles, assigned and unassigned devices, Simple Queues, FastTrack exemptions, live telemetry, historical analytics, provider quotas (limit, thresholds, ISP modem/portal URLs), and monitored WAN interfaces. Switching routers does not bleed quotas or settings from other hardware. **Device-identity heuristics respect the boundary too**: MAC-rotation merge suggestions, adapter link suggestions and the automatic rotation-consolidation pass only ever match a device against users and devices *on the same router* — a shared DHCP hostname (`NamasT3k`, `iPhone`) on two different routers is two machines, never a cross-router link. **The router's UTC offset is stored per router** (`router_gmt_offset_minutes_<id>`), so daily rollup dates, billing-cycle boundaries and the 1D history view follow each router's own clock rather than whichever router last streamed telemetry; the Router Health graphs (`/api/v1/metrics/system`, `/api/v1/metrics/interfaces`) return timestamps already shifted to that router's local wall-clock, matching the header.
  * **Live Router Switching**: Seamlessly switch active router contexts from the top navigation selector or the Settings modal. The dashboard, device inboxes, user cards, and WebSocket telemetry stream update instantly to the selected router. The selector's status dot reads a router as online whenever its telemetry stream is connected, not only when the 30-second `/routers` probe last succeeded, so a reachable but slow remote router is not shown as offline. That probe uses a dedicated short-lived client (10 s timeout, its own circuit breaker) so a slow status check cannot trip the pooled telemetry connection.
  * **Editable Connection Details**: A saved router's host, port, transport, username and password can be corrected in place from *Settings → Routers*. This matters most after a factory reset, which drops the router's certificate, its REST user and its password at once and leaves a stored record that can no longer connect. The stored password is never returned by the API, so the field is left blank and a blank field means *keep the current one*; **Test Connection** stays disabled until a password is typed, because testing with an empty one registers on the router as a failed login for that user.
  * **Change Router (hardware swap)**: The ↻ button on a router row opens the add-router form; on a successful test-connection to the **new** box its host/port/credentials/serial are written onto the *existing* row, so every user, device, traffic total and per-router setting stays attached — no re-entry. The old router is never contacted, so it works when the old one is already dead. A serial that already belongs to another live router is refused (`409`). At swap time the admin chooses whether the previous hardware's CPU / temperature / interface graphs (`system_metrics`, `interface_metrics`, `speed_test_results`) are kept or cleared for the new silicon; gateway and per-user/-device traffic totals are always kept (`POST /api/v1/routers/{id}/change`, `history_mode: keep | reset_hardware`).
  * **Delete with a choice**: Deleting a router (🗑) asks whether to **Archive** or **Delete permanently**. *Archive* (`mode=archive`, the default) sets `routers.archived_at`, hides the router from the picker, the Settings list and every background loop, and keeps every user, device, rollup, metric and `*_<id>` setting untouched — adding the same box again (matched on RouterBoard `serial_number`, read from `/system/routerboard`) **restores** that row instead of creating a second one. *Delete permanently* (`mode=purge`, gated behind typing the router's name) removes the router and explicitly deletes all of that data in one transaction — necessary because the SQLite deployment runs with `PRAGMA foreign_keys=OFF`, so the models' `ON DELETE CASCADE` never fires. Archived routers are listed under *Settings → Routers → Archived routers* with **Restore** and **Delete permanently** actions (`GET /api/v1/routers/archived`, `POST /api/v1/routers/{id}/restore`).
  * **Auto-Configure SSL**: Automatically generates a TLS certificate directly on RouterOS and enables the HTTPS REST API. Provisioning (self-signed, bind-existing, or upload-PEM) only flips `www-ssl` on and binds the certificate — it **never writes `port`, `address`, or any other `/ip/service` field**. The port `www-ssl` is bound to is read back from the router (`/ip/service`), reported in the provisioning response, and used to repoint MikroMan's own connection, so a custom `www` / `www-ssl` port set by the administrator (e.g. `444`) is preserved. On a successful HTTP connection test, if `www-ssl` is already enabled the wizard offers a one-click switch to HTTPS on that exact port.
...
* **🎨 Modern Responsive Web Dashboard:**
  ...
  * **Compact Telemetry Strip**: Download, Upload, CPU, RAM and Temperature carry inline **sparklines** built from the live telemetry stream (no extra requests, no charting dependency), alongside the **WAN IP**, the **Users tile** and uptime. Temperature is coloured against your configured warning threshold.
  * **Users tile**: leads with the number of profiles on this router and carries their device count underneath as *online / total* (`user_count`, `client_device_count`, `active_clients` on the telemetry frame) — "how many people, and how much of their hardware is up right now" in one tile. Clicking it opens Users & Devices.
  ...
  * **WAN Identity Tile**: shows the interface address, the address the internet actually sees (they differ under carrier-grade NAT), and the **provider name** — since an AS number and operator name are the only way to tell two links apart when both hand out CGNAT addresses. Failure is silent: the router may legitimately have no internet.
    * **Per router, from the router itself**: the public address is read from that router's `/ip/cloud` (`public-address`, which RouterOS keeps current over DDNS), so a multi-router install shows each site's *own* uplink — not the container's egress, which is only one of them. A container-side echo (`ipinfo.io` / `api.ipify.org`) is the fallback for a router that can't say (DDNS never reached, or a `0.0.0.0` / CGNAT / RFC1918 `public-address`). The result is cached **per router for 15 minutes**, so an upstream or routing change on the provider's side is picked up on its own within the window.
    * **Recognisable brand, not the legal entity**: the operator is looked up for that specific address via `ipwho.is` (HTTPS) — the brand is the operator's own domain's registrable label (`ucell.uz` → `Ucell`), never `COSCOM Liability Limited Company`; `ip-api.com`'s address form is the plaintext fallback for the name only. The address and AS number always come from the HTTPS sources.

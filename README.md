...
* **📈 Peak-Preserving Hardware & Bandwidth Graphs**:
  ...
  * Rates are summed per sample before the peak is taken, so a multi-interface selection cannot invent a combined spike out of two unrelated moments. The bucket rows carry the true per-instant summed peak (`rx/tx_rate_sum_bps_max`) taken at write time, because that peak cannot be reconstructed from per-interface aggregates: a sum of peaks overstates it, a max of peaks understates it.
